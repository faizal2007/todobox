...
def achievements():
    ...
    try:
        ...
        stats = {
            ...
            'completion_rate': round((total_completed / total_todos * 100), 1) if total_todos > 0 else 0,
            'average_completion_time': 0
        }
        
        # Get first batch of completed todos
        ...
        app.logger.error(f"Error loading achievements page: {str(e)}")
...
def achievements_batch():
    ...
    try:
        ...
        return jsonify({
            'items': items,
            'count': len(items),
            'has_more': len(completed_todos) > batch_size,  # If we got more than batch size, there's more
