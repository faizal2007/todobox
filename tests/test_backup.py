...
def test_backup_functionality():
    ...
    with app.app_context():
        ...
        with app.test_client() as client:
            # Make unauthenticated backup request (expect redirect or error)
            _ = client.get('/backup?format=json',
                           headers={'Authorization': f'Bearer {test_user.api_token}'} if test_user.api_token else None)

            # Authenticate and test endpoints without nesting client contexts
